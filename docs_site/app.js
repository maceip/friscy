    ...
    term.onData((data) => {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (code === 3) {
                ...
                outBytes.push(...Array.from(bytes));
                break;
            } else {
        ...
        const pendingReq = !!controlView && Atomics.load(controlView, 0) === CMD_STDIN_REQUEST;
        if (pendingReq) {
            stdinQueue.push(...outBytes);
            checkStdinRequest();
            return;
        }

        if (worker) {
            const payload = new Uint8Array(outBytes);
            worker.postMessage({ type: 'stdin-push', data: payload }, [payload.buffer]);
            return;
        }

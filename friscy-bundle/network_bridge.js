...
export class FriscyNetworkBridge {
  ...
  scheduleReconnect() {
    ...
    if (this._reconnectAttempts > 5) {
      console.warn('[friscy-net] Giving up after 5 reconnect attempts. Network unavailable.');
      return;
    }
    this.reconnecting = true;

    const delay = Math.min(2000 * this._reconnectAttempts, 10000);
    console.log(`[friscy-net] Reconnecting in ${delay/1000}s (attempt ${this._reconnectAttempts}/5)...`);
    setTimeout(() => {
      this.reconnecting = false;
      this.connect().catch(() => {});
    }, delay);

...
let processEventLog = [];
let processEventPanelEl = null;
const processEventPanelEnabled = new URLSearchParams(window.location.search).has('procdebug')
    || new URLSearchParams(window.location.search).has('process_events');
...
function ensureProcessEventPanel() {
    ...
    panel.innerHTML = '<div style="font-weight:600;margin-bottom:6px;color:#f8fafc">friscy process events</div>' +
        '<div id="friscy-process-events-list"></div>';
...
function handleWorkerRuntimeMessage(e) {
    ...
    if (msg.type === 'checkpoint-exported-live') {
        ...
        a.download = `${activeExample}-snapshot.ckpt`;
        a.click();
    ...
    if (msg.type === 'process-events') {
        ...
        (window as any).__friscyProcessEvents = events;
        appendProcessEvents(events);
        (window as any).__friscyProcessEventLog = processEventLog;
    ...
    if (exportEventsBtn) {
        exportEventsBtn.addEventListener('click', () => {
            const payload = {
                ...
                url: location.href,
                processEvents: (window as any).__friscyProcessEventLog || processEventLog || [],
            };
...
const activeExample = pageParams.get('example') || 'alpine';

...
async function handleCheckpointUpload(req, res, pathname) {
  ...
  const tmpPath = path.join(BASE_DIR, `${safeName}.uploading`);
  const finalPath = path.join(BASE_DIR, safeName);
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      const done = finish(() => resolve(total));
      const fail = finish((err) => reject(err));

      req.on('data', (chunk) => {
        total += chunk.length;
        if (total > maxBytes) {
          try { req.destroy(new Error('request body too large')); } catch {}
        }
      });

...
const (
	...
	heapAddrBits = (_64bit*(1-goarch.IsWasm)*(1-goos.IsIos*goarch.IsArm64)*(1-goarch.IsRiscv64))*48 + (1-_64bit+goarch.IsWasm)*(32-(goarch.IsMips+goarch.IsMipsle)) + 40*goos.IsIos*goarch.IsArm64 + _64bit*goarch.IsRiscv64*35
...
func mallocinit() {
	...
	if goarch.PtrSize == 8 {
		...
		for i := 0x7f; i >= 0; i-- {
			...
			case GOARCH == "riscv64":
				// For emulators with limited address space (e.g. 2GB),
				// place heap hints in the low region: 0x10000000-0x70000000.
				// Each hint is spaced 16MB apart (i << 24), starting at 256MB.
				if i > 0x06 {
					continue
				}
				p = uintptr(0x10000000) + uintptr(i)<<24
			default:

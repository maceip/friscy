...
const cmdHistory = {
    ...
    push(cmd) {
        ...
        this._pos = -1;
        try { localStorage.setItem(HISTORY_KEY, JSON.stringify(this._entries)); } catch {}
    },
    up() {
...
function setupDragDrop(terminalEl, term) {
    ...
    imageSidePanelClose.addEventListener('click', () => {
        ...
        droppedImagePreview.src = ''; // Clear image
    });
...
\x1b[36m[drop] ${file.name} (${(file.size/1024).toFixed(1)}KB)\x1b[0m`);
                ...
                droppedImagePreview.src = dataURL;
...
let term2, fitAddon2, worker2;
let machineRunning2 = false;
let controlSab2, stdoutSab2, netSab2, controlView2, stdoutView2, stdoutBytes2;
const stdinQueue2 = [];
...
function updateJitWarmupHud(stats) {
    ...
    jitHudCompiledEl.textContent = String(compiled);
    jitHudQueueEl.textContent = String(queueDepth);
    jitHudMissEl.textContent = formatPercent(missRate);
    jitHudPredictEl.textContent = formatPercent(predictorHitRate);
...
function drainStdout() {
    ...
    // copy to regular Uint8Array first
...
// --- Parameterized helpers for dual terminal ---
function drainStdoutFor(sv, sb, t) {
    if (!sv || !sb || !t) return;
    const writeHead = Atomics.load(sv, 0);
    const readTail = Atomics.load(sv, 1);
    if (writeHead === readTail) return;
    let available;
    if (writeHead >= readTail) available = writeHead - readTail;
    else available = RING_SIZE - readTail + writeHead;
    if (available <= 0) return;
    const buf = new Uint8Array(available);
    let pos = readTail;
    for (let i = 0; i < available; i++) {
        buf[i] = sb[RING_HEADER + pos];
        pos = (pos + 1) % RING_SIZE;
    }
    Atomics.store(sv, 1, pos);
    t.write(new TextDecoder().decode(buf));
}

function checkStdinRequestFor(cv, cs, queue) {
    if (!cv) return;
    const cmd = Atomics.load(cv, 0);
    if (cmd !== CMD_STDIN_REQUEST) return;
    if (queue.length === 0) return;
    const maxLen = Atomics.load(cv, 2);
    const controlBytes = new Uint8Array(cs);
    const toSend = Math.min(queue.length, maxLen, 3968);
    for (let i = 0; i < toSend; i++) {
        controlBytes[64 + i] = queue.shift();
    }
    Atomics.store(cv, 2, toSend);
    Atomics.store(cv, 0, CMD_STDIN_READY);
    Atomics.notify(cv, 0);
}

function checkExitFor(cv, t, statusText) {
    if (!cv) return false;
    const cmd = Atomics.load(cv, 0);
    if (cmd === CMD_EXIT) {
        const exitCode = Atomics.load(cv, 5);
        t.writeln(`
\x1b[33mProcess exited (code ${exitCode})\x1b[0m`);
        Atomics.store(cv, 0, CMD_IDLE);
        return true;
    }
    return false;
}
...
async function main() {
    ...
    if (activeExample === 'server') {
        const [serverRootfs, alpineRootfs] = await Promise.all([
            fetchWithProgress(rootfsUrl),
            fetch('./rootfs.tar').then(r => r.arrayBuffer()),
        ]);
        rootfs = serverRootfs;
        companionRootfs = alpineRootfs;
    } else {
    ...
    // WebGL renderer (GPU-accelerated, 3-10x faster)
    try {
    ...
    // Inline images (sixel + iTerm2 IIP protocol)
    try {
    ...
    const pollTimer = setInterval(() => {
        ...
        if (checkExit()) {
            ...
            // cleanup (VH networking is stateless — no teardown needed)
        }
    ...
    // Pass env vars from manifest as --env KEY=VALUE args
    // Merge default env + example-specific env (example overrides matching keys)
    ...
    statusEl.innerHTML = 'fast risc-v runtime for the browser &amp; wasm' + netStatusHTML;

    // Server tab: boot companion Alpine shell + auto wget
    if (activeExample === 'server' && companionRootfs) {
        bootCompanion(companionRootfs, proxyUrl).then(() => runAutoWget());
    }
...
// Boot
main().catch(e => {
    console.error('[friscy] main() failed:', e);
    const s = document.getElementById('progress-stage');
